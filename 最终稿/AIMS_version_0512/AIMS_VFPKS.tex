...
Inspired by the approach in \cite{othmer2000diffusion,othmer2002diffusion}, we first show, by formal arguments, that the system \eqref{eq: starting model equation} can be obtained in the diffusion limit of a nonlinear kinetic equation, provided that both the transport term and the turning operator are density-dependent { (Theorem \ref{theorem1} in Section \ref{sec:macroscopic derivation})}. The corresponding kinetic velocity-jump model we propose reads 
\begin{equation}
...
Thus far, we could perform the derivation without specifying the equation for the chemoattractant $c$. From the remaining of the paper, we choose the production and consumption of chemoattractant $g(\rho,c)$ in Eq. \eqref{eq: inital model without scaling} to be linear in both variables $g(\rho,c) = \rho - c$. Therefore, $c$ solves:
...
Following a gradient flow approach to energy in the sense of Refs.~\cite{de2018energy,calvez2006volume,carrillo2001entropy}, we start by defining $H(\rho) = \frac{D_0}{\chi_0}\ln\left(\frac{\rho}{q(\rho)}\right)$. %that plays the role of the nonlinear diffusion, namely, 
...
Suppose that $c(t,\mathbf{x})$ and $\rho(t,\mathbf{x})$ solve Eqs. \eqref{elliptic_c}-\eqref{eq: new version of macro}. We have
\begin{equation}
...
To facilitate a more straightforward analysis of the asymptotic preserving property based on the micro-macro decomposition, we adopt the finite difference discretization. This choice enables a fully explicit formulation and simplifies the theoretical verification of the asymptotic preserving property.
...
\end{proof}
The CFL condition (37) derived is not sharp. In practice, violating this condition does not necessarily lead to computational breakdown. Nevertheless, it provides useful guidance on the choice of time steps for ensuring stability.

\section{Numerical experiments}\label{numerical results}

In this section we present several numerical examples.
In particular, we {numerically verify} the convergence of the kinetic model proposed in \eqref{eq:kinetic_approx} (numerically solved with \eqref{eq:scheme} from the micro-macro decomposition \eqref{eq:macro_micro}), which we denote as $\rho_\textnormal{kinetic}^\e$, to the volume-exclusion Keller-Segel model \eqref{eq:macro_proliferation} (numerically solved with \eqref{scheme:limit}), denoted as $\rho_\textnormal{macro}$, as $\varepsilon\to 0$ in one and two dimensions. 
...
The convergence of the numerical patterns towards $\varepsilon\to0$ is illustrated in Fig. \ref{fig:2D_convergence_sol}. 
...
\caption{Comparison of $\rho_\textnormal{kinetic}^{\varepsilon}(t, \bx)$ for $\varepsilon=10^{-1},10^{-2},10^{-3}$ and $\rho_\textnormal{macro}(t, \bx)$ at $T=20$. In the numerical tests, we set $\chi_0=20$ for the first and third rows, and $\chi_0=50$ for the second and fourth rows. Additionally, the initial values are approximately 0.5 for the first and second rows, and around 0.1 for the third and fourth rows.}
...
From the modeling perspective, it would be natural to extend the derivation to consider different turning kernels, to take into account cell-cell adhesion or nonlocal movement, for instance. The idea to construct the scheme could be generalized to include these cases, but we stress the fact that the detailed discretization is problem-dependent. 
...
\bibitem{de2018energy} % 
\newblock L. Almeida, F. Bubba, B. Perthame and C. Pouchol, 
\newblock \doititle{Energy and implicit discretization of the Fokker-Planck and Keller-Segel type equations},
\newblock \emph{Netw. Heterog. Media}, \textbf{14} (2019), 23-41.

\bibitem{almeida2020treatment} % 
\newblock L. Almeida, G. Estrada-Rodriguez, L. Oliver, D.  Peurichard, A. Poulain and F. Vallette, 
\newblock \doititle{Treatment-induced shrinking of tumour aggregates: {A} nonlinear volume-filling chemotactic approach},
\newblock \emph{J. Math. Biol.}, \textbf{83} (2021).

\bibitem{Alt1980} % 
\newblock W. Alt, 
\newblock \doititle{Biased random walk models for chemotaxis and related diffusion approximations},
\newblock \emph{J. Math. Biol.}, \textbf{9} (1980), 147-177.

\bibitem{Brener1999} % 4
\newblock M. Brenner, P. Constantin, L. Kadanoff, A. Schenkel and S. Venkataramani,
\newblock \doititle{Diffusion, attraction and collapse},
\newblock \emph{Nonlinearity}, \textbf{12} (1999), 1071-1098.

\bibitem{calvez2006volume} % 5
\newblock V. Calvez and J. A. Carrillo, 
\newblock \doititle{Volume effects in the Keller-Segel model: energy estimates preventing blow-up}, 
\newblock \emph{J. Math. Pure. Appl.}, \textbf{86} (2006), 155-175.

\bibitem{carrillo2001entropy} %
\newblock J. A. Carrillo, A. J{\"u}ngel, P. A. Markowich, G. Toscani and A. Unterreiter, 
\newblock \doititle{Entropy dissipation methods for degenerate parabolic problems and generalized Sobolev inequalities},
\newblock \emph{Monatsh. Math.}, \textbf{133} (2001), 1-82.

\bibitem{carrillo2013asymptotic} %
\newblock J. A. Carrillo and B. Yan, 
\newblock \doititle{An asymptotic preserving scheme for the diffusive limit of kinetic systems for chemotaxis},
\newblock \emph{Multiscale Model. Sim.}, \textbf{11} (2013), 336-361.

\bibitem{chalub2004kinetic} % incollection
\newblock F. A. Chalub, P. A. Markowich, B. Perthame and C. Schmeiser, 
\newblock Kinetic models for chemotaxis and their drift-diffusion limits,
\newblock \emph{Nonlinear Differential Equation Models},  Springer, 2004, 123-141.

\bibitem{chertock2019high} % incollection
\newblock A. Chertock and A. Kurganov, 
\newblock High-resolution positivity and asymptotic preserving numerical methods for chemotaxis and related models,
\newblock \emph{Active Particles, Volume 2},  Springer, 2019, 109-148. 

\bibitem{dolak2005keller} 
\newblock Y. Dolak and C. Schmeiser,   
\newblock \doititle{The Keller-Segel model with logistic sensitivity function and small diffusivity},
\newblock \emph{SIAM J. Appl. Math.}, \textbf{66} (2005), 286-308.

\bibitem{emako2016well}  % ArXiv
\newblock C. Emako and M. Tang,   
\newblock \doititle{Well-balanced and asymptotic preserving schemes for kinetic models},
\newblock preprint, 2016, \arXiv{1603.03171}.

\bibitem{filbet2010class} 
\newblock F. Filbet and S. Jin,   
\newblock \doititle{A class of asymptotic-preserving schemes for kinetic equations and related problems with stiff sources},
\newblock \emph{J. Comput. Phys.}, \textbf{229} (2010), 7625-7648.

\bibitem{han2017pattern} 
\newblock Y. Han, Z. Li, J. Tao and M. Ma,    
\newblock \doititle{Pattern formation for a volume-filling chemotaxis model with logistic growth},
\newblock \emph{J. Math. Anal. Appl.}, \textbf{448} (2017), 885-907.

\bibitem{Herrero1998} % 14
\newblock M. Herrero, E. Medina and J. Velazquez, 
\newblock \doititle{Self-similar blow-up for a reaction-diffusion system},
\newblock \emph{J. Comp. Appl. Math.}, \textbf{97} (1998), 99-119.

\bibitem{Hillen2009} 
\newblock T. Hillen and K. Painter,   
\newblock \doititle{A user's guide to {PDE} models for chemotaxis},
\newblock \emph{J. Math. Biol.}, \textbf{58} (2009), 183-217.

\bibitem{horstmann2003} % 16
\newblock D. Horstmann, 
\newblock \doititle{From 1970 until present: the Keller-Segel model in chemotaxis and its consequences
i.},
\newblock \emph{Jahresberichte DMV}, \textbf{105} (2003), 103-165.

\bibitem{horstmann2004} % 17
\newblock D. Horstmann, 
\newblock \doititle{From 1970 until present: the Keller-Segel model in chemotaxis and its consequences
ii.},
\newblock \emph{Jahresberichte DMV}, \textbf{106} (2004), 51-69.

%\bibitem{Hwang2005} % 
%\newblock H. J. Hwang, K. Kang and A. Stevens,  
%\newblock \doititle{From 1970 until present: the Keller-Segel model in chemotaxis and its consequences
%ii.},
%\newblock \emph{Jahresberichte DMV}, \textbf{106} (2004), 51-69.

\bibitem{ibrahim2014efficacy} % 
\newblock M. Ibrahim and M. Saad,  
\newblock \doititle{On the efficacy of a control volume finite element method for the capture of patterns for a volume-filling chemotaxis model},
\newblock \emph{Comput.  Math. Appl.}, \textbf{68} (2014), 1032-1051.

\bibitem{jin1999efficient} % 19
\newblock S. Jin, 
\newblock \doititle{Efficient asymptotic-preserving (AP) schemes for some multiscale kinetic equations},
\newblock \emph{SIAM J. Sci. Comput.}, \textbf{21} (1999), 441-454.

\bibitem{jin2011class} % 
\newblock S. Jin and B. Yan, 
\newblock \doititle{A class of asymptotic-preserving schemes for the Fokker-Planck-Landau equation},
\newblock \emph{J. Comput. Phys.}, \textbf{230} (2011), 6420-6437.

\bibitem{Jin2008} % 
\newblock T. Jin, X. Xu and D. Hereld, 
\newblock \doititle{Chemotaxis, chemokine receptors and human disease},
\newblock \emph{Cytokine}, \textbf{44} (2008), 1-8.

\bibitem{Kaupp2012} % 
\newblock U. B. Kaupp, 
\newblock \doititle{100 years of sperm chemotaxis},
\newblock \emph{J. Gen. Physiol.}, \textbf{406} (2012), 583-586.

\bibitem{keller1970initiation} % 
\newblock E. F. Keller and L. A. Segel, 
\newblock \doititle{Initiation of slime mold aggregation viewed as an instability},
\newblock \emph{J. Theor. Biol.}, \textbf{26} (1970), 399-415.

\bibitem{keller1971model} % 
\newblock E. F. Keller and L. A. Segel, 
\newblock \doititle{Model for chemotaxis},
\newblock \emph{J. Theor. Biol.}, \textbf{30} (1971), 225-234.

\bibitem{LemouMieussens} % 
\newblock M. Lemou and L. Mieussens, 
\newblock \doititle{A new asymptotic preserving scheme based on micro-macro formulation for linear kinetic equations in the diffusion limit},
\newblock \emph{SIAM J. Sci. Comput.}, \textbf{31} (2008), 334-368.

\bibitem{ma2012stationary} % 
\newblock M. Ma, C. Ou and Z.-A. Wang, 
\newblock \doititle{Stationary solutions of a volume-filling chemotaxis model with logistic growth and their stability},
\newblock \emph{SIAM J. Appl. Math.}, \textbf{72} (2012), 740-766.

\bibitem{Meacock2021} % 27
\newblock O. Meacock, A. Doostmohammadi, K. Foster, J. Yeomans and W. M. Durham,
\newblock \doititle{Bacteria solve the problem of crowding by moving slowly.},
\newblock \emph{Nat. Phys.}, \textbf{17} (2021), 205-210.

\bibitem{murray2002introduction} % book -- 2002?
\newblock J. D. Murray, 
\newblock Mathematical biology I: An introduction,
\newblock \emph{Mathematical biology}, Springer, 2002.

\bibitem{murray2003mathematical} % book -- 2003? 
\newblock J. D. Murray, 
\newblock Mathematical biology II: Spatial models and biomedical applications
\newblock \emph{Mathematical biology}, Springer, 2003.

\bibitem{Nagai1997} % 30 proceedings
\newblock T. Nagai, 
\newblock \doititle{Global existence of solutions to a parabolic system for chemotaxis in two space dimensions},
\newblock \emph{Proceedings of the Second World Congress of Nonlinear Analysts}, \textbf{30}, 5381-5388.

\bibitem{Othmer1997} % 31
\newblock H. G. Othmer and A. Stevens, 
\newblock \doititle{Aggregation, blowup and collapse: The ABC’s of taxis in reinforced random walks},
\newblock \emph{SIAM J. Appl. Math.}, \textbf{} (1997), 1044-1081.

\bibitem{othmer2000diffusion} % 32
\newblock H. G. Othmer and T. Hillen, 
\newblock \doititle{The diffusion limit of transport equations derived from velocity-jump processes},
\newblock \emph{SIAM J. Appl. Math.}, \textbf{61} (2000), 751-775.

\bibitem{othmer2002diffusion} % 33
\newblock H. G. Othmer and T. Hillen, 
\newblock \doititle{The diffusion limit of transport equations ii: Chemotaxis equations},
\newblock \emph{SIAM J. Appl. Math.}, \textbf{62} (2002), 1222-1250.

\bibitem{painter2002volume} % 
\newblock K. J. Painter and T. Hillen, 
\newblock \doititle{Volume-filling and quorum-sensing in models for chemosensitive movement},
\newblock \emph{Can. Appl. Math. Q.}, \textbf{10} (2002), 501-543.

\bibitem{patlak1953random} % 
\newblock C. S. Patlak, 
\newblock \doititle{Random walk with persistence and external bias},
\newblock \emph{B. Math. Biophys.}, \textbf{15} (1953), 311-338.

\bibitem{Roussos2011} % 
\newblock E. Roussos, J.  Condeelis and A. Patsialou, 
\newblock \doititle{Chemotaxis in cancer},
\newblock \emph{Nat. Rev. Cancer}, \textbf{11} (2011), 573-587.

\bibitem{stevens2000derivation} % 37
\newblock A. Stevens, 
\newblock \doititle{The derivation of chemotaxis equations as limit dynamics of moderately interacting stochastic many-particle systems},
\newblock \emph{SIAM J. Appl. Math.}, \textbf{61} (2000), 183-212.

\bibitem{Vital2011} % 38
\newblock F. G. Vital-Lopez, A. Armaou, M. Hutnik and C. D. Maranas, 
\newblock \doititle{Modeling the effect of chemotaxis on glioblastoma tumor progression},
\newblock \emph{AIChE J.}, \textbf{57} (2011), 778-792.

\bibitem{wang2019asymptotic} % 
\newblock L. Wang and B. Yan, 
\newblock \doititle{An asymptotic-preserving scheme for the kinetic equation with anisotropic scattering: heavy tail equilibrium and degenerate collision frequency},
\newblock \emph{SIAM J. Sci. Comput.}, \textbf{41} (2019), A422-A451.

\bibitem{wang2010chemotaxis} % 
\newblock Z.-A. Wang, 
\newblock \doititle{On chemotaxis models with cell population interactions},
\newblock \emph{Math. Model. Nat. Pheno.}, \textbf{5} (2010), 173-190.

\bibitem{wang2007classical} % 
\newblock Z. Wang and T. Hillen, 
\newblock \doititle{Classical solutions and pattern formation for a volume filling chemotaxis model},
\newblock \emph{Chaos}, \textbf{17} (2007), 037108.

\bibitem{wrzosek2010volume} % 42
\newblock D. Wrzosek, 
\newblock \doititle{Volume filling effect in modelling chemotaxis},
\newblock \emph{Math. Model. Nat. Pheno.}, \textbf{5} (2010), 123-147.

\bibitem{zheng2015boundedness} % 43
\newblock P. Zheng, C. Mu and X. Hu, 
\newblock \doititle{Boundedness and blow-up for a chemotaxis system with generalized volume-filling effect and logistic source},
\newblock \emph{Discret. Contin. Dyn. S.}, \textbf{35} (2015), 2299.
