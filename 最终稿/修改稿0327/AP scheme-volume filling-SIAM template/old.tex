...
The PKS original model has been modified by various authors, with the aim of improving its consistency with biological systems. One example is the volume-exclusion chemotactic system introduced by Hillen and Painter \cite{painter2002volume} to take into account the finite size of cells and volume limitations. In such models the chemotactic sensitivity (i.e. the term leading to cell aggregation) depends on both the chemical concentration in the medium and the local cell density, thus, the population density directly modulates its own sensitivity response. The coupled system reads, in {{its}} parabolic-elliptic form as
\begin{equation}
\begin{aligned}
    \partial_t\rho-\nabla\cdot\left(D_0(q(\rho)-\rho q'(\rho))\nabla\rho-\chi_0q(\rho)\rho\nabla c\right)&=h(\rho,c)\ , \ \quad t\geq 0\ , \mathbf{x}\in \Omega\subset\mathbb{R}^n\ ,\\
    D_c\Delta c+g(\rho,c)& = 0\ .
    \label{eq: starting model equation}
\end{aligned}
\end{equation}
Here, $\rho(t,\mathbf{x})$ is the cell density, $c(t,\mathbf{x})$ is the chemoattractant concentration and  $q(\rho)$ is a function that describes the packing capacity of the cell aggregates.  The diffusion coefficients for the cells and chemoattractant are $D_0$ and $D_c$, respectively, and $\chi_0$ is the chemotactic sensitivity. The proliferation (or death) of the cells is described by $h(\rho,c)$. that in later section we are going to choose as a density-dependent logistic growth function,  and the production and consumption of the chemoattractant is given by $g(\rho,c)$. Note that the classical PKS model is recovered by taking $q(\rho) = 1$ in \cref{eq: starting model equation}.  It has been shown \cite{wrzosek2010volume,calvez2006volume,zheng2015boundedness} that such volume-exclusion effect prevents blow-ups in finite time compared to the model without density effects (with $q(\rho) = 1$). The volume-exclusion chemotactic equation has been widely studied in
...
it has proven to be successful at describing aggregation phenomena \cite{almeida2020treatment,bubba2019chemotaxis}. 
...
A natural question that arises is whether the volume-exclusion Keller-Segel equation \cref{eq: starting model equation} proposed in 
 \cite{painter2002volume} could be obtained as the diffusion limit of a kinetic `velocity jump' model \cite{othmer1988models}, giving insights into how the individual mechanisms at the cell level can lead to volume-exclusion effects at the population level. In this paper, inspired by the approach followed in \cite{othmer2002diffusion,othmer2000diffusion}, we first show, by formal arguments, that the system \cref{eq: starting model equation} can be obtained in the diffusion limit of a nonlinear kinetic equation, provided that both the transport term and the turning operator are density-dependent { (\cref{theorem1} in \Cref{sec:macroscopic derivation})}. The corresponding kinetic `velocity jump' model we propose reads 
\begin{equation}
    \partial_t f+\mathbf{v}\cdot \nabla(F[\rho](t,\mathbf{x},\mathbf{v}) f)= \lambda q(\rho)(-f+\rho T(\mathbf{v},\rho,\nabla c)) {\ +\ r_0f\Bigl(1-\frac{\rho}{\rho_{\textnormal{max}}} \Bigr)_+}\ ,\label{eq: inital model without scaling}
\end{equation} where $f(t,\mathbf{x},\mathbf{v}) \geq 0$ is the phase space cell density,  $\mathbf{x}\in \Omega \subset \mathbb{R}^n$ denotes the position, $\mathbf{v} \in V \subset \mathbb{R}^n$ is the velocity where $V$ denotes the unit sphere  $V=\{\mathbf{x}\in\mathbb{R}^n:\ |\mathbf{x}|=1 \}$,  $t \in \mathbb{R}^+$ the time,  $\rho_{\textnormal{max}}$ is the carrying capacity and $r_0$ is the proliferation rate.
Here, $\lambda$ is the constant turning rate, with $1/\lambda$ giving a measure of the mean run length between velocity jumps, $T(\mathbf{v},\rho,\nabla c)$ gives the probability of a velocity jump to velocity $\mathbf{v}$, which depends on the chemical concentration $c$ and the term $F[\rho](t,\mathbf{x},\mathbf{v})$ describes the anisotropic transport due to the density limited motion. {\DP Our model is reminiscent of existing models of the litterature \cite{othmer2002diffusion,othmer2000diffusion}. In particular in \cite{othmer2002diffusion}, the authors study the diffusion-limit of transport equations of type \eqref{eq: inital model without scaling}, under a variety of external biases imposed on the turning operator. Depending on the strength of the bias, the authors show that this leads to anisotropic diffusion, drift term in the flux, or both. In particular, they show that the Patlak-Keller-Segel-Alt (PKSA) only arises if the perturbation of the turning kernel is sufficiently small. They consider several examples of prototype models for chemotaxis of bacteria, of slime molds and of myxobacteria and present a general discussion of the derivation of diffusion approximations from various stochastic processes that model chemotaxis (see \cite{othmer2000diffusion} and references therein). Model \eqref{eq: inital model without scaling} aims to take a step further and study the case when cell motion is not only biased by external factors (leading to chemotactic motion) but also biased to account for density-limited motion/crowding effects. Model \eqref{eq: inital model without scaling} shows that the minimal hypothesis on the run-and-tumble process enabling to obtain the volume-exclusion KS system \eqref{eq: starting model equation} in the diffusion limit are density-dependent transport term and turning kernel. The first hypothesis (density-dependent transport) is the most natural in terms of a modelling viewpoint, and expresses that cells will only be able to move in not already crowded regions. The second hypothesis (density-dependent turning kernel) is a more 'active' mechanism expressing that cells have less probability to turn towards crowding regions. Applied to bacteria, these hypothesis are consistent with experimental evidences showing that tightly packed colonies could collectively move faster by reducing the speed of their individual bacteria \cite{Meacock2021}.} 

We then numerically investigate whether the relevant macroscopic volume exclusion equation corresponds to the underlying physical system described by the kinetic equation { in the diffusion limit}. These methods are widely known as asymptotic preserving schemes (AP) \cite{filbet2010class,jin1999efficient} since they mimic the asymptotic behaviour of the kinetic equation when the scaling parameter approaches to zero and the mesh size and time steps are fixed. In this paper, we use a micro-macro decomposition of the unknown in the sense of  \cite{LemouMieussens} as detailed in \Cref{sec:micro-macro}. {The finite difference discretization is explained in \Cref{sec: implicit-explicit scheme}, where a proper implicit-explicit discretization of the nonlinear terms, including the chemosensitivity term, is applied to improve the efficiency and stability of our numerical scheme.} This decomposition of the solution of the kinetic equation is analogous to a Chapman-Enskog expansion in the case of the classical Boltzmann equation. It uses the properties of the ``collisional operator'', which in our formulation describes the run and tumble movement of the individual. For the kinetic counterpart of the classical PKS equation we refer to \cite{carrillo2013asymptotic}, where the authors used an odd-even splitting at the kinetic level and studied {\XR the blow-up behaviour of solutions.}
...
\paragraph{The volume-exclusion PKS for modelling tumor growth} A relevant example\newline  where volume-exclusion effects play a crucial role is in the modelling of glioblastoma (GBM) aggregates  \cite{Vital2011}. In particular, the  system \cref{eq: starting model equation} was used in \cite{almeida2020treatment} to explain the mechanical changes at the cell level  due to the presence of a chemical treatment. %{In real-world applications,} mathematical models provide useful tools towards identifying links between phenomena observed at the macroscopic level and the underlying microscopic properties. Chemotactic models have been extensively used to describe glioblastoma (GBM) aggregates  \cite{Vital2011} and in particular, the volume-exclusion system \cref{eq: starting model equation} was used in \cite{almeida2020treatment} to explain the mechanical changes at the cell level in these systems due to the presence of a chemical treatment. 
In this work, the cell's elasticity is modelled through the term $q(\rho)$ which incorporates the cell-cell interactions \cite{wang2007classical}. This function $q(\rho)$ can be explicitly written as $q(\rho)=1-(\rho/\bar{\rho})^\gamma$ where $\gamma$ is a parameter that depends on the concentration of the treatment and $\bar{\rho}$ is the maximum cell density in each aggregate. For $\gamma=1$ the cells are considered as solid particles while {$\gamma>1$  corresponds to semi-elastic particles} that can squeeze into empty spaces.


A drawback of this approach is that it is formulated directly at the continuous (PDE) level, therefore the interactions comprised in such models are mostly based on phenomenological considerations at the population level. However, neglecting the microstructural features causes these models to fail to predict the micromechanical behaviors, which makes the validation of these approaches {difficult} due to the lack of physical mechanisms to interpret the population macroscopic behavior. Establishing the link between macroscopic models and their microscopic counterparts, the central task of kinetic theory, can benefit macroscopic models gain in predictive character and become invaluable aids for experimental data analysis. {This paper aims to take a step in this direction, by providing an interpretation of the volume-filling chemotactic system as the diffusion limit of a kinetic {{`velocity jump'}} model which gives a more direct interpretation of the PDE operators in terms of more fundamental characteristics of the motion.} {We then validate this limit using an AP numerical scheme.}
...
\begin{equation*}
    \partial_t f+\mathbf{v}\cdot \nabla(F[\rho](t,\mathbf{x},\mathbf{v}) f)= \lambda q(\rho)(-f+\rho T(\mathbf{v},\rho,\nabla c)){\ +\ r_0f\Bigl(1-\frac{\rho}{\rho_{\textnormal{max}}} \Bigr)_+}\ .
\end{equation*}
...
$$
F[\tilde{\rho}](\tau,\mathbf{\xi},\mathbf{u}) = {{q(\tilde{\rho}(\tau, \mathbf{\xi} + \frac{s}{L} \mathbf{u}))}}\ .
$$
...
\end{equation*}}
where $\langle \bar{T}\rangle=0$. Note that the error terms contained in $\mathcal{O}(\varepsilon^2)$ integrate to 0 in the velocity variable. Using \cref{expansion_psi}, the turning kernel $T_\varepsilon$ writes
\begin{align}
...
\end{align}
{where the $\mathcal{O}(\varepsilon^3)$-term {is such that} $\langle\mathcal{O}(\varepsilon^3)\rangle=0$. We also note that using Hypotheses \cref{hypo1} and \cref{hypo2} we have $\langle T_\varepsilon\rangle=1$ which describes the conservation of individuals during the velocity reorientation.}
...
\end{align}
Integrating \cref{eq: epsilon2} with respect to $\mathbf{v}\in V$ and noticing that $\langle \bar{T}\rangle=0$, using Hypothesis \cref{hypo1}, we get
\begin{equation}
...
\begin{equation*}
    \partial_t\rho+\nabla\cdot(\chi_0 q(\rho)\rho(-H'(\rho)\nabla\rho+\nabla c))=h(\rho),
\end{equation*}
{\XR where a proliferation term $h(\rho)$ is included satisfying $h(\rho)\ge0$ and $h(\rho)H(\rho)\le0$. 
In fact, with a similar argument as in the proof of \Cref{prop:energy}, we have 
\begin{align*}
...
{\XR 
The transport term $F_\varepsilon(\rho)$ and the turning operator $T_\varepsilon$ introduced in \cref{TepsFeps} are highly nonlinear and inconvenient for practical use.
As we are interested in the limit of small $\varepsilon$, for simplicity, from now on we will consider a slightly modified version of the kinetic model by truncating $F_\varepsilon$ and $T_\varepsilon$ to the first order in the expansions \cref{eq: F scaled} and \cref{eq: turning operator scaled}.
To be exactly, we solve the following equation }
\beq
...
\end{align*}
With a similar argument as in the proof of \cref{theorem1}, we can show that the approximate generalized kinetic model \cref{eq:kinetic_approx} converges to the same macroscopic model \cref{eq: macroscopic equation1} in the diffusive limit $\varepsilon\to 0$.
    % \beq
    % \label{eq:limit}
...
{\XR To get the equation for $\rho$, we substitute the micro-macro decomposition  \cref{def:f_decompose} into the modified model \cref{eq:kinetic_approx} and integrate over $\bv$. 
Noticing that 
\begin{align*}
...
\end{align*}
Then taking the operator $I-\Pi$ on equation \cref{eq:kinetic_approx}, we get
\begin{align}
...
In fact, the leading order term in \cref{eq:g_macro} shows that 
\begin{equation*}
...
\end{equation*}
Substituting it into \cref{eq:rho_macro}, we recover  
\begin{equation}
...
By discretizing the system  \cref{eq:macro_micro} via a finite difference method, we will get an asymptotic preserving scheme, which will be formally proven later in the section. 
...
in an upwind manner and thus helps improve {\XR the numerical stability when discretizing the macroscopic model \cref{eq: macroscopic equation1}. 
To guarantee that the numerical scheme designed for the micro-macro formulation \cref{eq:macro_micro} shares the same stability property in the limit $\varepsilon\to 0$, we keep using $\Phi^{n_1,n}_{j+\frac12}$ to approximate $q(\rho)\rho$ appearing in \cref{eq:macro_micro}. 
}
...
\end{equation}
which can be easily derived by comparing \cref{eq:tilde_g} and the second equation in \cref{eq:scheme}.  
...
{Here, we formally check the asymptotic preserving property of the scheme by taking $\varepsilon \to 0$ in the system \cref{eq:scheme}. We only need to show that the scheme for the kinetic model \cref{eq:kinetic_approx} converges to a scheme for the corresponding macroscopic model \cref{eq: macroscopic equation1}}, or the first equation in  \cref{eq:macro_proliferation}. 
...
which is indeed the finite difference system for solving \cref{eq:macro_proliferation}. 
In this way, we verified the {{asymptotic preserving}} property of our scheme \cref{eq:scheme}. 
...
\end{equation}
\noindent where $\Gamma = \underset{s\in [\rho_{\max}, \bar{\rho}]}{\inf} \frac{\bar{\rho} - s}{\bar{\rho} q(s)}$ {\XR and $\eta_{j+1/2}= ...$}. This CFL condition ensures that if $0 \leq \rho_j^0 \leq \bar{\rho}$ for all $i$ then $0 \leq \rho_j^n \leq \bar{\rho}$ for all $j$ and $n$.
}
\begin{proposition} 
...
\noindent Noticing that $\eta_{j+\frac12}^n\Phi_{j+\frac12}^{n+1,n}=(\eta_{j+\frac12}^n)_+ q(\rho_{j+1}^n)\rho_j^{n+1} - (\eta_{j+\frac12}^n)_- q(\rho_{j}^n)\rho_{j+1}^{n+1}$ via \cref{def:Phi}, the numerical scheme \cref{scheme:limit} can be reformulated in the matrix form
...
{\color{red}\Delta t \leq \frac{\Delta x}{2 \max_j |\eta_{j+1/2}|}\ \underset{ \rho_{\max} < \rho_j^n < \bar{\rho}}{\min} \ \frac{\bar{\rho} - \rho_j^n}{\bar{\rho} q(\rho_j^n)}.}
\end{equation}
...
\end{equation}
\noindent with $ \Phi $ satisfing Eq. \cref{functionPhi}. 
{\XR Unfortunately, it is difficult to prove the energy dissipation property theoretically for the numerical scheme. 
Instead, we will show it numerically for both the macro model and the kinetic model. 
}
{Via numerical integration, we can accurately approximate $\Phi(\rho)$. The energy $\mathcal{E}(t)$ in \cref{def:energy} can then be numerically approximated via quadrature rules.} 
...
\begin{equation}\label{def:psi01}
    \psi_0(v) = \frac{1}{\sqrt{2\pi}}e^{-\frac{v^2}{2}}\ , \quad \psi_1(v, {{\p_x c}}) = \frac{v}{\sqrt{2\pi}}e^{-\frac{v^2}{2}} {\color{red}{\p_x c}}\ .
\end{equation}
{It is easy to check that the choices of $\psi_0(v)$ and $\psi_1(v)$ satisfy the Hypothesis \cref{hypo1} and Hypothesis  \cref{hypo2}.}

{\XR Though the numerical scheme \cref{eq:scheme} works well in the diffusion limit $\varepsilon\to0$, it is conditionally stable and numerical experiments show that the restrictions on time step $\Delta t$ becomes stronger as $\varepsilon$ increases. 
To avoid unexpected breakdown during the long time simulation, for all the test cases $\varepsilon = 0.2$, 0.1 and 0.05, we fix the time step to be relatively small. 
}
By choosing the time step $\Delta t = 10^{-4}$ and the initial data
\begin{align*}
...
in the kinetic model, which could result in a delay in the aggregation process. In the next section, we take a step further and analyse the evolution of the pattern sizes in time as function of the chemotactic sensitivity  $\chi_0$.}
...
\caption{Left: Evolution of the {relative} $L_2$-error $\frac{\|\rho_{\rm{macro}}(t, \cdot) - \rho_{\rm{kinetic}}(t,\cdot)\|_2}{\|\rho_{\rm{macro}}(t, \cdot)\|}$ over time with $\chi_0=20$. Right: Convergence of the relative $L_2$-error in $\e$  at $t=10,\  20, \ 50$. The numerical setting is the same as in \cref{fig:density_eps_A_20}.}
...
For the volume-exclusion Keller-Segel model \cref{eq:macro_proliferation}, the relation between the aggregate size from {\XR an initial data perturbed from some homogeneous steady state} and the strength of chemotaxis effect $\chi_0$ {was proven in \cite{almeida2020treatment} via linear stability analysis. }
{\XR
...
In this section we perform 2D simulations for both  {{the kinetic model \cref{eq:kinetic_approx}}} and  {{the volume-exclusion Keller-Segel model \cref{eq:macro_proliferation}}}. 
...
In \cref{fig:2D_macro} we show the numerical results at $t=5$ (first row), $t=20$ (second row) and $t=50$ (third row) for the macroscopic model with different chemotaxis sensitivities $\chi_0=6$ (first column), $\chi_0=20$ (second column) and $\chi_0=50$ (third column). As one can observe, starting from an initial data perturbed around the homogeneous value $0.5$, we obtain the formation of labyrinthic patterns for a chemotactic sensitivity $\chi_0>6$ (middle and right columns), while the solution dampens to the homogeneous state for $\chi_0=6$ (left column), in agreement with the predictions of the stability analysis performed in \cite{almeida2020treatment} and the results in \cref{fig:k2_inv_compare}. Moreover, we observe that larger values of the chemotactic sensitivity $\chi_0$ {lead} to sharper layers near the boundary of the patterns (compare middle and right columns) as expected. 

In \cref{fig:2D_compare} we compare the solutions of the macroscopic 2D model (top row) with the solutions of the 2D kinetic model for $\varepsilon = 10^{-2}$ (bottom row), for two values of the chemotactic sensitivity $\chi_0 = 20$ (first and third columns) and $\chi_0 = 50$ (second and fourth columns), and for different values of the initial data $\rho_0$: $\rho_0 = 0.5$ (first two columns) and $\rho_0 = 0.1$ (last two columns). As observed in \cite{painter2002volume}, we recover the formation of different types of patterns as a function of the initial condition for both the kinetic and the macro solution, i.e. labyrinthic patterns in the case $\rho_0 = 0.5$ and round patterns for $\rho_0 = 0.1$ (compare the first two columns with the last two). Moreover, we observe that the pattern sizes decrease and become sharper when the chemotactic sensitivity $\chi_0$ increases also for the kinetic solution.% in the case of round patterns (last two columns Fig. \cref{fig:2D_compare}), and again that the boundary of the patterns become sharper in the case of labyrinthic patterns (compare the first two columns) also for the kinetic solution. %These results are in very good agreement with the predictions of the stability analysis, and observed for both the macroscopic model and the kinetic one with $\varepsilon = 10^{-2}$, which are in very good agreement (compare the two columns). 
...
{\DP
\section{Energy dissipation of the numerical scheme in the limit $\epsilon \rightarrow 0$ (TO FINISH)}

Here, we aim to show that the limiting scheme \eqref{scheme:limit} is energy dissipative. To this aim, we define the fully discrete free energy at time $t^n$ by
$$
E[\rho^n] = \sum_{i=1}^N \Delta x \Phi(\rho_i^n) - \frac{1}{2} \sum_{i=1}^N \Delta x \rho_i^n c_i^n.
$$
We compute:
\begin{equation*}
\begin{split}
E[\rho^{n+1}] - E[\rho^n] &= \sum_i \Delta x (\Phi(\rho_i^{n+1}) - \Phi(\rho_i^n) - \frac{1}{2} \sum_i \Delta x (\rho_i^{n+1} - \rho_i^n) c_i^n\\
& = \sum_i \Delta x (\Phi(\rho_i^{n+1}) - \Phi(\rho_i^{n}) - (\rho_i^{n+1} - \rho_i^n)\Phi'(\rho_i^{n+1})) \\
&+ \sum_i \Delta x (\rho_i^{n+1} - \rho_i^n) \big( \Phi'(\rho_i^{n+1}) - \frac{1}{2} c_i^n ).
\end{split}
\end{equation*}
(Remark: As a first step, I only worked with $\rho$ for fixed $c^n$, it is however possible to consider $- \frac{1}{2} \sum_{i=1}^N \Delta x (\rho_i^{n+1} c_i^{n+1} - \rho_i^n c_i^n)$ and use the equation for $c$ to obtain $\frac{1}{2} \sum_{i=1}^N \Delta x (\rho_i^{n+1} c_i^{n+1} - \rho_i^n c_i^n) = \sum_{i=1}^N \Delta x \frac{c_i^{n+1} + c_i^n}{2}(\rho_i^{n+1} - \rho_i^n)$). 

The first term in the last equality of $E(\rho^{n+1}) - E(\rho^n)$ is negative because of the convexity of function $\Phi$. For the second term, using Eq. \eqref{scheme:limit}:
\begin{equation*}
\begin{split}
II = \sum_i \Delta x (\rho_i^{n+1} - \rho_i^n) \big( \Phi'(\rho_i^{n+1}) - \frac{1}{2} c_i^n ) &= - \Delta t \sum_i (F_{i+1/2}^{n+1} - F_{i-1/2}^{n+1}) \big( \Phi'(\rho_i^{n+1}) - \frac{1}{2} c_i^n ) \\
&= - \Delta t \sum_i F_{i+1/2}^{n+1} \big( \Phi'(\rho_i^{n+1}) - \Phi'(\rho_{i+1}^{n+1})  -\frac{1}{2} (c_i^n - c_{i+1}^{n} )\\
&=  \Delta t \sum_i F_{i+1/2}^{n+1} \big( \Phi'(\rho_{i+1}^{n+1}) - \Phi'(\rho_i^{n+1})  -\frac{1}{2} ( c_{i+1}^{n} - c_i^n)
\end{split}
\end{equation*}
where
\begin{equation*}
\begin{split}
F_{i+1/2}^{n+1} &= - D_h d(\bar{\rho}_i^n) \frac{\rho_{i+1}^{n+1} - \rho_i^{n+1}}{\Delta x} + <v_k \psi_k> \frac{c_{i+1}^{n} - c_i^{n}}{\Delta x} \phi_{i+1/2}^{n+1,n} \\
&= <v_k \psi_k>\phi_{i+1/2}^{n+1,n} \bigg(- \frac{D_h d(\bar{\rho}_i^n)}{<v_k \psi_k> \phi_{i+1/2}^{n+1,n}} \frac{\rho_{i+1}^{n+1} - \rho_i^{n+1}}{\Delta x} +  \frac{c_{i+1}^{n} - c_i^{n}}{\Delta x} )\bigg)
\end{split}
\end{equation*}
So now we run into a problem. Usually what we have is that the flux $F_{i+1/2}^{n+1}$ can be expressed as something negative ($-<v_k \psi_k>\phi_{i+1/2}^{n+1,n}$) multiplied by $\Phi'(\rho_{i+1}^{n+1}) - \Phi'(\rho_i^{n+1})  +\frac{1}{2} (c_i^n - c_{i+1}^{n})$, so that multiplying them lead directly to something negative multiplied by a square (positive) and we get negativity of the term II (this is what is done in the paper of Bailo and José for instance). Note that this is the discrete equivalent to the term $|\nabla (H-c)|^2$ when we prove energy dissipation for the macro model.  However for our scheme although $ \frac{D_h d(\bar{\rho}_i^n)}{<v_k \psi_k> \phi_{i+1/2}^{n+1,n}} \frac{\rho_{i+1}^{n+1} - \rho_i^{n+1}}{\Delta x}$ is indeed an approximation of $\Phi'(\rho_{i+1}^{n+1}) - \Phi'(\rho_i^{n+1})$, it cannot be put directly on the form $\Phi'(\rho_{i+1}^{n+1}) - \Phi'(\rho_i^{n+1})$ because we evaluate the function $d$ in $\bar{\rho}_i^n$. So we need to work out the arguments to say that II is negative. From the convexity of $\Phi$ we know that $\Phi'$ is monotonically non decreasing, i.e $\Phi'(\rho_{i+1}^{n+1}) - \Phi'(\rho_i^{n+1})$ is of the same sign as $\rho_{i+1}^{n+1} - \rho_i^{n+1}$, that means that at least we know that $- \frac{D_h d(\bar{\rho}_i^n)}{<v_k \psi_k> \phi_{i+1/2}^{n+1,n}} \frac{\rho_{i+1}^{n+1} - \rho_i^{n+1}}{\Delta x}$ and $\Phi'(\rho_{i+1}^{n+1}) - \Phi'(\rho_i^{n+1})$ have the same sign. That's great, but we still cannot conclude that II is negative because of the extra term $c_{i+1}^{n} - c_i^{n}$ in the parenthesis. 

Another line of thought would be to write:
\begin{equation*}
\begin{split}
II &=  -\Delta t \sum_i \frac{<v_k \psi_k>\phi_{i+1/2}^{n+1,n}}{\Delta x} A^2\\
& +\Delta t \sum_i \frac{<v_k \psi_k>\phi_{i+1/2}^{n+1,n}}{\Delta x} AB\\
\end{split}
\end{equation*}
where 
$$A = \bigg(\frac{D_h d(\bar{\rho}_i^n)}{<v_k \psi_k> \phi_{i+1/2}^{n+1,n}} (\rho_{i+1}^{n+1} - \rho_i^{n+1}) - \frac{1}{2} (c_{i+1}^{n} - c_i^n) \bigg)$$
and
$$
B = \bigg(\frac{D_h d(\bar{\rho}_i^n)}{<v_k \psi_k> \phi_{i+1/2}^{n+1,n}} (\rho_{i+1}^{n+1} - \rho_i^{n+1}) - (\Phi'(\rho_{i+1}^{n+1}) - \Phi'(\rho_i^{n+1}))
$$
and show that $B$ is small in any case. Ideas? 
}
