...
\end{aligned}
\end{equation}
Here, $\rho(t,\mathbf{x})$ is the cell density, $c(t,\mathbf{x})$ is the chemoattractant concentration and  $q(\rho)$ is a function that describes the packing capacity of the cell aggregates.  The diffusion coefficients for the cells and chemoattractant are $D_0$ and $D_c$, respectively, and $\chi_0$ is the chemotactic sensitivity. The proliferation (or death) of the cells is described by $h(\rho,c)$,  and the production and consumption of the chemoattractant is given by $g(\rho,c)$. The dependency of $h$ and $g$ on the cell density $\rho$ enables to account for density-limited phenomena such as the logistic-growth function for $h$ commonly used in the literature \cite{murray2002introduction,murray2003mathematical}. Note that from \eqref{eq: starting model equation}, we recover the classical PKS model by taking $q(\rho) = 1$.  It has been shown \cite{wrzosek2010volume,calvez2006volume,zheng2015boundedness} that such volume-exclusion effect prevents blow-ups in finite time compared to the model without density effects (with $q(\rho) = 1$). The volume-exclusion chemotactic equation has been widely studied in the literature, from a modelling \cite{painter2002volume,wang2010chemotaxis}, analytic \cite{wrzosek2010volume,han2017pattern,ma2012stationary,dolak2005keller} and numerical perspectives \cite{ibrahim2014efficacy}. A relevant example where volume-exclusion effects play a crucial role is in the modelling of glioblastoma (GBM) aggregates  \cite{Vital2011}. In particular, the  system \cref{eq: starting model equation} was used in \cite{almeida2020treatment} to explain the mechanical changes at the cell level  due to the presence of a chemical treatment. In \cite{almeida2020treatment}, the cell's elasticity is modelled through the term $q(\rho)$ which incorporates the cell-cell interactions \cite{wang2007classical}. This function $q(\rho)$ was chosen to be $q(\rho)=1-(\rho/\bar{\rho})^\gamma$ where $\gamma$ is a parameter that depends on the concentration of the treatment and $\bar{\rho}$ is the maximum cell density in each aggregate. For $\gamma=1$ the cells were considered as solid particles while {$\gamma>1$  corresponded to semi-elastic particles} that can squeeze into empty spaces. 

A drawback of studying volume-exclusion effects directly at the continuous (PDE) level is that the interactions comprised in such models are mostly based on phenomenological considerations at the population level.  However, neglecting the microstructural features makes these models difficult to interpret in terms of mechanisms at the cellular level. \textcolor{red}{In one dimension, a simplified model for $\gamma=1$ was derived in \cite{painter2002volume} from a discrete-space random walk. In Section 
\ref{sec:model} we use a similar approach to generalise it to higher dimensions for $\gamma>1$.} Establishing the link between macroscopic models and their microscopic counterparts, the central task of kinetic theory, can benefit macroscopic models gain in predictive character and become invaluable aids for experimental data analysis. {This paper aims to take a step in this direction, by providing an interpretation of the volume-filling chemotactic system \cref{eq: starting model equation} as the diffusion limit of a kinetic {{`velocity jump'}} model which gives a more direct interpretation of the PDE operators in terms of more fundamental characteristics of the motion.} 

%A natural question that arises is whether the volume-exclusion Keller-Segel equation proposed in 
 %\cite{painter2002volume} could be obtained as the diffusion limit of a kinetic `velocity jump' model \cite{othmer1988models}, giving insights into how the individual mechanisms at the cell level can lead to volume-exclusion effects at the population level. 
Inspired by the approach followed in \cite{othmer2002diffusion,othmer2000diffusion}, we first show, by formal arguments, that the system \cref{eq: starting model equation} can be obtained in the diffusion limit of a nonlinear kinetic equation, provided that both the transport term and the turning operator are density-dependent { (\cref{theorem1} in \Cref{sec:macroscopic derivation})}. The corresponding kinetic `velocity jump' model we propose reads 
\begin{equation}
...
{ For simplicity, we consider the case without proliferation, i.e. $r_0 = 0$ in \cref{eq: macroscopic equation1}. 
}
...
{ To get the equation for $\rho$, we substitute the micro-macro decomposition  \cref{def:f_decompose} into the modified model \cref{eq:kinetic_approx} and integrate over $\bv$. 
Noticing that 
\begin{align*}
...
in an upwind manner and thus helps improve { the numerical stability when discretizing the macroscopic model \cref{eq: macroscopic equation1}. 
...
{ Though the scheme \cref{eq:rho_n} might not be positive preserving for a general fixed $\dt > 0$ and $\e>0$, 
...
{ Though the numerical scheme \cref{eq:scheme} works well in the diffusion limit $\varepsilon\to0$, it is conditionally stable and numerical experiments show that the restrictions on time step $\Delta t$ becomes stronger as $\varepsilon$ increases. 
{\color{red} One primary cause of instability is that the AP scheme does not preserve boundedness. 
As illustrated in Fig. \ref{fig:stab_1D}, we checked the boundedness of numerical solutions at $T=20$ for various choices of $\varepsilon$ and $\Delta t$. 
Generally, a smaller time step $\Delta t$ is required for large values of $\varepsilon$. 
Conversely, for small $\varepsilon$, a larger $\Delta t$ may be selected to achieve the same stability as the limiting scheme for the macro model. 
}
\begin{figure}[tbhp]
\centerline{\includegraphics[scale = 0.5]{stab_test.eps}
}
\caption{{\color{red}Stability of the AP scheme. We checked the boundedness of  numerical solutions at $T=20$, computed for various choices of $\varepsilon$ and $\Delta t$. Solutions that remain within $[0,1]$ are denoted by squares, whereas those outside this range are indicated by triangles.}}
\label{fig:stab_1D}
\end{figure}
...
For the volume-exclusion Keller-Segel model \cref{eq:macro_proliferation}, the relation between the aggregate size from { an initial data perturbed from some homogeneous steady state} and the strength of chemotaxis effect $\chi_0$ {was proven in \cite{almeida2020treatment} via linear stability analysis. }
{
...
{ With the numerical experimental settings in \Cref{subsec:energy_dissipation}, here we compare the numerical results with the following formula, which was proposed in \cite{almeida2020treatment}, 
$$
...
It explains why in \cref{fig:k2_inv_compare} the good agreement between the predicted pattern sizes and the numerical ones can be observed at $t=1$ when $\chi_0$ is large while the same phenomenon for small $\chi_0$ can only be observed when $t=20$. 
}
...
{\color{red} The convergence of the numerical patterns towards $\varepsilon\to0$ is illustrated in Fig. \ref{fig:2D_convergence_sol}.}
...
\begin{figure}[tbhp]
\centerline{
\includegraphics[scale = 0.2]{Fig_2D_A_20_eps_1e+00_T_20_init_1.eps}
...
\centerline{
\includegraphics[scale = 0.2]{Fig_2D_A_50_eps_1e+00_T_20_init_1.eps}
...
\centerline{
\includegraphics[scale = 0.2]{Fig_2D_A_20_eps_1e+00_T_20_init_10.eps}
...
\centerline{
\includegraphics[scale = 0.2]{Fig_2D_A_50_eps_1e+00_T_20_init_10.eps}
...
\caption{{\color{red}Comparison of $\rho_\textnormal{kinetic}^{\varepsilon}(t, \bx)$ at $T=20$ with $\varepsilon=1,10^{-1},10^{-2},10^{-3}$ and $\chi_0=20, 50$.
The initial value is chosen to be around 0.5.}}
